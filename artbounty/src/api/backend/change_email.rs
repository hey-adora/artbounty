...
use crate::api::{
    ...
    Server404Err, ServerAddPostErr, ServerAuthErr, ServerDecodeInviteErr, ServerDesErr, ServerErr,
    ServerErrImg, ServerErrImgMeta, ServerLoginErr, ServerRegistrationErr, ServerReq, ServerRes,
    ServerTokenErr, User, UserPost, UserPostFile, auth_token_get, hash_password, verify_password,
};
...
pub async fn change_email(
    ...
    let email_change = app
        ...
        .await
        .map_err(|e| match e {
            DB404Err::NotFound => ServerErr::from(ServerErr::from(ResErr::InvalidStage(format!(
                "state not initialized, expected ReadyToConfirm"
            )))),
            DB404Err::DB(_) => ServerErr::DbErr,
        })?;
    ...
    let result = app
        ...
        .await
        .map_err(|err| match err {
            DBChangeEmailErr::EmailIsTaken(email) => ServerErr::from(ResErr::EmailIsTaken(email)),
            _ => ServerErr::DbErr,
        })?;
